...
class WordPressSource {
  ...
  async getMenus (store) {
    ...
    for (const menu of data) {
      const fields = this.normalizeFields(menu)
  ...
  async downloadImage (url, destPath, fileName) {
    ...
    const protocol = this.options.baseUrl.substring(0, 5) === 'https' ? https : http
  ...
  normalizeFields (fields, isACF) {
    ...
    for (const key in fields) {
      ...
      res[camelCase(key)] = this.normalizeFieldValue(fields[key], isACF || key.startsWith('acf_'))
    }
  ...
  normalizeFieldValue (value, isACF) {
    ...
    if (isPlainObject(value)) {
      ...
      } else if (value.post_type === 'nav_menu_item' && (value.ID || value.id)) {
